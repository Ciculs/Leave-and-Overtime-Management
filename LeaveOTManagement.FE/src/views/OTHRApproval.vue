  ...
  <div class="ot-container">
    ...
    <main>
      ...
      <div v-if="totalPages > 1" class="pagination">
        ...
        <button v-for="p in totalPages" :key="p" class="page-number" :class="{ active: currentPage === p }"
          @click="currentPage = p">
          {{ p }}
        </button>
        ...
        <div v-if="selectedOT" class="modal-overlay" @click.self="selectedOT = null">
          <div class="modal-card">
            ...
            <div class="timeline">
              ...
              <div class="timeline-item" :class="{ done: selectedOT.status === 'Approved' }">
                <div class="dot"></div>
                HR Decision
              </div>
            </div>
            ...
            <div v-if="selectedOT.userApprovalStatus === 'Pending' && selectedOT.status === 'ManagerApproved'"
              ...
              <button class="reject-btn" @click="reject(selectedOT.id)">
                Reject
              </button>
            </div>
          </div>
        ...
        <div v-if="showRejectModal" class="modal-overlay">
          <div class="modal-card">
            ...
            <div class="modal-actions">
              <button class="approve-btn" style="background:#64748b" @click="closeReject">
                Cancel
              </button>
...
const filteredOT = computed(() => {
  ...
  if (selectedStatus.value) {
    if (selectedStatus.value === "Approved") {
      data = data.filter(x => x.status === "Approved" || x.status === "HRApproved")
    } else {
      data = data.filter(
  ...
  data.sort((a, b) => {
    ...
    const hourA = Number(calculateHours(a.details?.[0]))
    const hourB = Number(calculateHours(b.details?.[0]))
